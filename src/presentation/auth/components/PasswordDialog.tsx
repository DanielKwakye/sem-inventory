...
export function PasswordDialog({ onLoginSuccess, systemReady }: Props) {
        ...
        <Dialog onOpenChange={(open) => {
            ...
            <form>
                ...
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        <DialogTitle>{ !systemReady ? "" : "Login As Admin"}</DialogTitle>
                        <DialogDescription>
                            {systemReady && (<span>Password is <strong className={"text-lg"}>"123456"</strong> for demo purposes</span>) }
                        </DialogDescription>
                    </DialogHeader>
                    { !systemReady ? (
